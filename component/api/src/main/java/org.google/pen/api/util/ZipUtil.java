...
package org.google.pen.api.util;
...
public class ZipUtil {
    ...
    public ZipArchive createZipFile(String owner, String deviceType, String deviceId, String deviceName,
        ...
        try {
            ...
            if (configuration != null && configuration.getConfiguration() != null && configuration
                ...
                for (ConfigurationEntry configurationEntry : configurations) {
                    switch (configurationEntry.getName()) {
                        case DeviceTypeUtilConstants.DEVICE_TYPE_HTTPS_EP:
                            httpsServerEP = (String)configurationEntry.getValue();
                            break;
                        case DeviceTypeUtilConstants.DEVICE_TYPE_HTTP_EP:
                            httpServerEP = (String)configurationEntry.getValue();
                            break;
                        case DeviceTypeUtilConstants.DEVICE_TYPE_MQTT_EP:
            ...
            contextParams.put(DeviceTypeUtilConstants.TENANT_DOMAIN, APIUtil.getTenantDomainOftheUser());
            contextParams.put(DeviceTypeUtilConstants.DEVICE_OWNER, owner);
            contextParams.put(DeviceTypeUtilConstants.DEVICE_ID, deviceId);
            contextParams.put(DeviceTypeUtilConstants.DEVICE_NAME, deviceName);
            contextParams.put(DeviceTypeUtilConstants.HTTPS_EP, httpsServerEP);
            contextParams.put(DeviceTypeUtilConstants.HTTP_EP, httpServerEP);
            contextParams.put(DeviceTypeUtilConstants.APIM_EP, httpServerEP);
            contextParams.put(DeviceTypeUtilConstants.MQTT_EP, mqttEndpoint);
            contextParams.put(DeviceTypeUtilConstants.API_APPLICATION_KEY, base64EncodedApplicationKey);
            contextParams.put(DeviceTypeUtilConstants.DEVICE_TOKEN, token);
            contextParams.put(DeviceTypeUtilConstants.DEVICE_REFRESH_TOKEN, refreshToken);
